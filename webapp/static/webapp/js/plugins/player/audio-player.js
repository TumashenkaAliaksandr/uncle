...
//         }, [{
// 			image : 'static/webapp/images/weekly/song1.jpg',
...
//         }, {
// 			image : 'static/webapp/images/weekly/song2.jpg',
//             title: "Your Face",
//             artist: "Ministry",
...
// 			option : myPlayListOtion
//         }, {
// 			image : 'static/webapp/images/weekly/song3.jpg',
//             title: "Cyber Sonnet",
//             artist: "You Am I",
...
// 			option : myPlayListOtion
//         }, {
// 			image : 'static/webapp/images/weekly/song4.jpg',
//             title: "Tempered Song",
//             artist: "Shelter",
...
// 			option : myPlayListOtion
//         }, {
// 			image : 'static/webapp/images/weekly/song5.jpg',
//             title: "Hidden",
//             artist: "Bad Religion",
...
// 			option : myPlayListOtion
//         }, {
// 			image : 'static/webapp/images/weekly/song6.jpg',
//             title: "Lentement",
//             artist: "Apollo 440",
...
// 			option : myPlayListOtion
//         }, {
// 			image : 'static/webapp/images/weekly/song7.jpg',
//             title: "Lismore",
...
// 			option : myPlayListOtion
//         }, {
// 			image : 'static/webapp/images/weekly/song8.jpg',
...
// 			option : myPlayListOtion
//         }, {
// 			image : 'static/webapp/images/weekly/song9.jpg',
//             title: "Beside Me",
//             artist: "Friendly Fires ",
...
$(function () {
    ...
    // Проверяем наличие плеера на странице
    if ($('.audio-player').length) {
        var myPlayListOtion = '<ul class="more_option"><li><a href="#"><span class="opt_icon" title="Add To Favourites"><span class="icon icon_fav"></span></span></a></li><li><a href="#"><span class="opt_icon" title="Add To Queue"><span class="icon icon_queue"></span></span></a></li><li><a href="#"><span class="opt_icon" title="Download Now"><span class="icon icon_dwn"></span></span></a></li><li><a href="#"><span class="opt_icon" title="Add To Playlist"><span class="icon icon_playlst"></span></span></a></li><li><a href="#"><span class="opt_icon" title="Share"><span class="icon icon_share"></span></span></a></li></ul>';

        // Загружаем песни через AJAX
        $.getJSON('/api/songs/', function (data) {
            var playlist = data.map(function (song) {
                return {
                    image: song.image,  // Используем поле image из JSON (photo_file.url)
                    title: song.title,  // Используем поле title из JSON
                    artist: song.artist,  // Используем поле artist из JSON (album.artist)
                    mp3: song.mp3,  // Используем поле mp3 из JSON (audio_file.url)
                    oga: song.oga,  // Используем поле oga из JSON (video_file.url)
                    option: myPlayListOtion  // Опции для песни
                };
            });

            // Инициализируем плеер с загруженным плейлистом
            var myPlaylist = new jPlayerPlaylist({
                jPlayer: "#jquery_jplayer_1",
                cssSelectorAncestor: "#jp_container_1"
            }, playlist, {
                ...
                keyEnabled: true,
                playlistOptions: {
                    autoPlay: false // Установите true, если хотите, чтобы первая песня играла автоматически
                }
            });

            // Обновляем отображение текущей песни при воспроизведении
            $("#jquery_jplayer_1").on($.jPlayer.event.ready + ' ' + $.jPlayer.event.play, function (event) {
                var current = myPlaylist.current;
                var playlist = myPlaylist.playlist;
                $.each(playlist, function (index, obj) {
            ...
            // Обработка событий для ползунка времени
            var timeDrag = false;
            $('.jp-play-bar').mousedown(function(e) {
                timeDrag = true;
                updatebar(e.pageX);
            ...
            // Обработка кликов по элементам плейлиста
            $('#playlist-toggle, #playlist-text, #playlist-wrap li a').unbind().on('click', function() {
                $('#playlist-wrap').fadeToggle();
                $('#playlist-toggle, #playlist-text').toggleClass('playlist-is-visible');
            });

            // Скрытие/показ плеера
            $('.hide_player').unbind().on('click', function() {
                $('.audio-player').toggleClass('is_hidden');
                $(this).html($(this).html() == '<i class="fa fa-angle-down"></i> HIDE' ? '<i class="fa fa-angle-up"></i> SHOW PLAYER' : '<i class="fa fa-angle-down"></i> HIDE');
            });
